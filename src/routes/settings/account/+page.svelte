<script lang="ts">
	...
	const { form: formEmail, errors: errorsEmail, enhance: enhanceEmail } = superForm(data.formEmail, {
	  resetForm: false,
	  applyAction: false,
	  onResult({ result }) {
	...
	const { form: formUsername, errors: errorsUsername, enhance: enhanceUsername } = superForm(data.formUsername, {
	  resetForm: false,
	  applyAction: false,
	  onResult({ result }) {
		if (result.status === 200) {
	...
	<div class="bg-surface-300-600-token w-full max-w-md p-8 shadow-md rounded-container-token flow">
	  ...
	  <form class="space-y-6 grid gap-2" use:enhanceEmail action="?/updateEmail" method="POST">
	  ...
	  <form class="space-y-6 grid gap-2" use:enhanceUsername action="?/updateUsername" method="POST">
		<label class="label" for="email">
			<span>Email</span>
			<input
