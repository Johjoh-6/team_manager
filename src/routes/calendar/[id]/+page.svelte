...
<section class="content-grid mt-4">
	<div class="event grid grid-cols-2 card p-4 gap-4 select-none">
		<h2 class=" text-center text-xl text-primary-500 capitalize">{event.name}</h2>
		<span class="badge bg-primary-500 p-2 text-white "
		>{event?.expand?.type.name ?? 'Non défini'}</span
		>
		<p class="font-semibold text-primary-500 text-center t-start">
			Début
		</p>
		<p class="start text-center">{dateFormatFr(event.date_start)}</p>
		<p class="t-end font-semibold text-primary-500 text-center">Fin </p>
		<p class="end text-center">{dateFormatFr(event.date_end)}</p>
		<p class="diff text-center">Évenement dans <strong class="text-primary-500">{dayBeforeEvent(event.date_start)}</strong> jour(s)</p>
		...
		{#if data.isManager}
		<div class="actions">
			<button class="btn bg-secondary-500 text-white" on:click={()=> {
